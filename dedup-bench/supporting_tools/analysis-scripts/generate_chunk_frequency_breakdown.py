...
def generate_shared_chunk_histogram_data(hash_reader: hashfilereader.HashFileReader) -> list:
    ...
    # количество совпадающих хешей
    equal_hash = 0
    total_chunk_number = 0
    for chunk_hash, chunk_props in chunk_properties.items():
        total_chunk_number += chunk_props['frequency']
        equal_hash += chunk_props['frequency'] - 1
    print(equal_hash)

    chunk_sizes = [0 for i in range(40000)]
    x = [i for i in range(40000)]
    for chunk_hash, chunk_props in chunk_properties.items():
        chunk_sizes[chunk_props['size']] += chunk_props['frequency']

    for i in range(40000):
        chunk_sizes[i] /= total_chunk_number

    return chunk_sizes, x
